...
describe('StorageManager', () => {
  ...
  const FULL_TEST_KEY = 'ngx-signal-plus:test-key';
  ...
  describe('Basic Operations', () => {
    ...
    it('should save and load complex objects', () => {
      ...
      const loaded: typeof value | undefined =
        StorageManager.load<typeof value>(TEST_KEY);
      expect(loaded).toEqual(value);
    });

    it('should handle undefined values', () => {
      const loaded: string | undefined =
        StorageManager.load<string>('non-existent-key');
    ...
    it('should handle numeric values', () => {
      ...
      values.forEach((value: number) => {
        ...
        const loaded: number | undefined =
          StorageManager.load<number>(TEST_KEY);
        expect(loaded).toBe(value);
      });
    });
  });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle JSON parse errors', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        jasmine.stringMatching(/Failed to load/),
      );
    });
  ...
  describe('Complex Scenarios', () => {
    it('should handle large objects', () => {
      const largeObject: {
        array: string[];
        nested: { deep: { deeper: { deepest: boolean } } };
      } = {
        array: new Array(1000).fill('test'),
        nested: { deep: { deeper: { deepest: true } } },
      };
      StorageManager.save(TEST_KEY, largeObject);
      const loaded: typeof largeObject | undefined =
        StorageManager.load<typeof largeObject>(TEST_KEY);
    ...
    it('should maintain type safety', () => {
      ...
      const loadedValue: TestType | undefined =
        StorageManager.load<TestType>(TEST_KEY);
    ...
    it('should handle array values', () => {
      ...
      const loadedArray: number[] | undefined =
        StorageManager.load<number[]>(TEST_KEY);
    ...
    it('should handle circular references', () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        jasmine.stringMatching(/Failed to save.*circular structure/i),
      );
  ...
  describe('Storage Key Management', () => {
    ...
    it('should handle invalid keys', () => {
      const invalidKeys: (string | null | undefined)[] = [
        '',
        ' ',
        null,
        undefined,
      ...
      invalidKeys.forEach((key: string | null | undefined) => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
          jasmine.stringMatching(/Invalid key/),
        );
      });
    });
    ...
    it('should handle very long keys', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        jasmine.stringMatching(/key length exceeds/),
      );
    });
  ...
  describe('Data Integrity', () => {
    ...
    it('should handle partial data corruption', () => {
      ...
      const corruptedJson: string | undefined = localStorage
        .getItem(FULL_TEST_KEY)
        ?.replace('test', 'test"');
    ...
    it('should handle versioned data structures', () => {
      ...
      const loaded: NewVersion | undefined =
        StorageManager.load<NewVersion>(TEST_KEY);
  ...
  describe('Concurrency', () => {
    it('should handle storage events from other contexts', (done) => {
      ...
      const fullKey = `ngx-signal-plus:${storageKey}`;
      ...
      setTimeout(() => {
        const loaded: string | undefined =
          StorageManager.load<string>(storageKey);
        expect(loaded).toBe(value);
        done();
      }, 0);
    });
