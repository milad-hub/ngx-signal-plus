...
export const presets = {
  ...
  toggle: (initial = false): SignalBuilder<boolean> => {
    return new SignalBuilder(initial).withHistory();
  },
  ...
  searchField: (initial = ''): SignalBuilder<string> => {
    return new SignalBuilder(initial).debounce(300).distinct().withHistory();
  },
  ...
  persistentToggle: (initial = false, key: string): SignalBuilder<boolean> => {
    return new SignalBuilder(initial).persist(key).withHistory();
  },
};
