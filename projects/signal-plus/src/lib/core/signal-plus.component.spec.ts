...
describe('SignalPlusComponent', () => {
  ...
  describe('component lifecycle', () => {
    it('should handle subscription cleanup', fakeAsync(() => {
      ...
      const subscription: () => void = signal.subscribe((value) =>
        values.push(value),
      );
  ...
  describe('performance and state consistency', () => {
    it('should handle rapid updates while maintaining state', fakeAsync(() => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const values: any[] = [];
      const unsubscribe: () => void = component.counter.subscribe((v) =>
        values.push(v),
      );
  ...
  describe('complex state management', () => {
    it('should maintain state consistency under rapid changes', fakeAsync(() => {
      const rapidChanges: { counter: number; input: string; amount: number }[] =
  ...
  describe('performance boundaries', () => {
    ...
    it('should maintain stable memory usage under load', fakeAsync(() => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        if (i % 100 === 0) {
          const memory: PerformanceMemory | undefined = (
            performance as unknown as { memory?: PerformanceMemory }
          ).memory;
      ...
      tick(300);
      const memoryVariance: number =
        Math.max(...memorySnapshots) - Math.min(...memorySnapshots);
  ...
  describe('browser integration', () => {
    it('should handle tab visibility changes', fakeAsync(() => {
      const visibilityHandler: jasmine.Spy =
        jasmine.createSpy('visibilityHandler');
  ...
  describe('theme persistence edge cases', () => {
    it('should handle initial theme load', fakeAsync(() => {
      ...
      const newFixture: ComponentFixture<SignalPlusComponent> =
        TestBed.createComponent(SignalPlusComponent);
  ...
  describe('theme persistence', () => {
    ...
    it('should handle theme toggle and persistence', fakeAsync(() => {
      ...
      const storedTheme = JSON.parse(
        localStorage.getItem('theme-mode') || 'false',
      );
