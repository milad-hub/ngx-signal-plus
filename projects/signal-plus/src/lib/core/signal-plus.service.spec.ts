...
describe('SignalPlusService', () => {
  ...
  describe('signal creation', () => {
    ...
    it('should create signal with validation', () => {
      ...
      const signal: NumberSignal = service
        .create(0)
    ...
    it('should create signal with persistence', fakeAsync(() => {
      const signal: SignalPlus<string> = service
        .create('test')
        .persist('test-key')
        .build();
    ...
    it('should create signal with history', () => {
      const signal: SignalPlus<number> = service
        .create(0)
        .withHistory(true)
        .build();
      signal.setValue(1);
    ...
    describe('number validators', () => {
      ...
      it('should validate number range', () => {
        const inRange: (value: number) => boolean =
          service.validators.number.range(0, 10);
    ...
    describe('string validators', () => {
      ...
      it('should validate email pattern', () => {
        const isEmail: (value: string) => boolean =
          service.validators.string.pattern(/^[^\s@]+@[^\s@]+\.[^\s@]+$/);
  ...
  describe('error handling', () => {
    it('should throw on undefined initial value', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect(() => service.create(undefined as any)).toThrowError(
    ...
    it('should throw on negative debounce time', () => {
      ...
      expect(errorHandler).toHaveBeenCalledWith(
        new Error('Debounce time must be positive'),
      );
    });
  ...
  describe('advanced validation scenarios', () => {
    ...
    it('should handle validation chain order', () => {
      ...
      try {
        ...
        // Expected error - testing validation order
      }
  ...
  describe('advanced history scenarios', () => {
    ...
    it('should handle history with complex objects', () => {
      ...
      const signal: SignalPlus<ComplexType> = service
        .create(initial)
        .withHistory()
        .build();
  ...
  describe('storage error scenarios', () => {
    it('should handle storage errors gracefully', fakeAsync(() => {
      const errorSpy: jasmine.Spy = jasmine.createSpy('errorSpy');
      const signal: SignalPlus<{ value: number }> = service
        .create({ value: 0 })
        .persist('storage-test')
        .onError(errorSpy)
        .build();
      const originalSetItem: jasmine.Spy = spyOn(
        localStorage,
        'setItem',
